...
for sample in mc :
    ...
    # irene tring to remove QCD unc from minor bkg
    # if sample in ["ggWW","VBS","VBS_dipoleRecoil","VVV", "VBF-V_dipole", "Vg", "VgS","VBS_dipoleRecoil"] + wjets_all_bins + VBS_samples + VV_samples + VBS_aQGC_samples: continue
...
# VV samples
# nuisances['QCD_scale_QCD_VV_accept'] = {
#             'name'  : 'QCDscale_QCD_VV_accept',
#             'kind'  : 'weight',
#             'type'  : 'shape',
...
#     # irene tring to remove PS unc from minor bkg
#     # if sample in VBS_samples + VV_samples + ["VBS","VBS_dipoleRecoil","VVV", "VBF-V_dipole", "Vg", "VgS","VBS_dipoleRecoil","ggWW"] : continue
...
nuisances['PS_ISR']  = {
                ...
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  :    dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mc if skey not in ["VBF-V_dipole"]),
            }
nuisances['PS_FSR']  = {
                'name'  : 'CMS_PS_FSR',
                'kind'  : 'weight',
...
# irene tring to remove pdf unc from minor bkg
# nuisances['pdf_weight'] = { # --> Now save also the normalization one for the signal
#     'name'  : 'pdf_1718',
#     'kind'  : 'weight_envelope',
#     'type'  : 'shape',
#     'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ["top","VVV", "VBF-V_dipole", "Vg", "VgS","VBS_dipoleRecoil","ggWW"]+wjets_all_bins+VV_samples},
#     'AsLnN':  '1'
# }
...
for wjbin in wjets_all_bins:
        if "boost" in wjbin:
            nuisances["{}_norm_boost_2018".format(wjbin)]  = {
                'name'  : 'CMS_{}_norm_boost_2018'.format(wjbin),
                'samples'  : {wjbin: '1.00'},
                'type'  : 'rateParam',
            }
