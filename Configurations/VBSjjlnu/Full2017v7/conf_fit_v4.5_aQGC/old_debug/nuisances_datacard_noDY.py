...
mc =["DY", "top", "VVV", "VBF-V_dipole", "Vg", "VgS","VBS_dipoleRecoil","ggWW", "Wjets_boost"] + wjets_res_bins + VV_samples + VBS_aQGC_samples
# "VBS", "VBF-V",
...
nuisances['eff_e']  = {
                ...
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  :   dict((skey, id_syst_ele) for skey in mc if skey not in ["DY"] ),
                'cuts': phase_spaces_tot_ele
}
...
nuisances['electronpt']  = {
                ...
                'mapDown': 'ElepTdo',
                'cuts': phase_spaces_tot_ele,
                'samples': dict((skey, ['1.','1.']) for skey in mc if skey not in ['Vg', 'VgS', 'ggWW',"DY"]),
...
nuisances['eff_m']  = {
                ...
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  : dict((skey, id_syst_mu) for skey in mc if skey not in ["DY"] ),
                'cuts': phase_spaces_tot_mu
}
...
for js in jes_systs:
    nuisances[js]  = {
                    ...
                    'mapUp': js+'up',
                    'mapDown': js+'do',
                    'samples': dict((skey, ['1.','1.']) for skey in mc if skey not in ["Vg","VgS", 'ggWW',"DY"]+VV_samples+VBS_aQGC_samples),
...
for sample in mc :
    if sample in ["ggWW","VBS","VBS_dipoleRecoil","Wjets_boost","DY"] + wjets_res_bins + VBS_samples + VV_samples + VBS_aQGC_samples: continue
...
nuisances['PS_ISR']  = {
                ...
                'kind'  : 'weight',
                'type'  : 'shape',
                'samples'  :    dict((skey, ['PSWeight[2]', 'PSWeight[0]']) for skey in mc if skey not in ["DY"] ),
            }
nuisances['PS_FSR']  = {
                'name'  : 'CMS_PS_FSR',
                'kind'  : 'weight',
...
nuisances['PU']  = {
                ...
                'samples'  : {
                    ...
                    s : ["",""] for s in mc if s not in ["DY"]}, # only for dataset and plotting 
                'AsLnN'      : '1',
}
...
nuisances['UE']  = {
                ...
                'skipCMS' : 1,
                'type': 'lnN',
                'samples': dict((skey, '1.015') for skey in mc if skey not in ["Wjets_boost","top","DY"]+wjets_res_bins), 
}
